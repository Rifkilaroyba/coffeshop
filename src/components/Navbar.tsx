...
const navItemClass = ({ isActive }: { isActive: boolean }) =>
  `px-3 py-2 rounded-full text-sm font-medium transition ${
    isActive
      ...
      : 'text-[rgb(var(--foreground))]/80 hover:bg-primary/10 hover:text-primary'
  }`
...
function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b border-black/5">
      <div className="max-w-7xl mx-auto container-px">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:block">
            <a href="#products" className="btn-primary">Belanja Kopi</a>
          </div>

          {/* Mobile toggle */}
          <button
            className="md:hidden inline-flex items-center justify-center rounded-md p-2 border border-black/10 hover:bg-black/5"
      ...
      {open && (
        <div className="md:hidden border-t border-black/5 bg-white/95 backdrop-blur">
          <div className="container-px py-3 space-y-2">
            ...
            <div className="pt-2">
              <a href="#products" className="btn-primary w-full justify-center">Belanja Kopi</a>
            </div>
          </div>
        </div>
      )}
